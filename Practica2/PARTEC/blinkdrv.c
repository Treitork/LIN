	...
	size_t len, loff_t *off)
{
	...
	int retval = 0, available_space = BUFFER_LENGTH-1, i=0;
	unsigned int color=0;
	
	/*Array de mensajes con la información de cada led*/
	unsigned char messages[NR_LEDS][NR_BYTES_BLINK_MSG];	
	
	char kbuf[BUFFER_LENGTH], *kbufp=kbuf, *led; 
	const char *separate=","; // separador
    ...
    if (copy_from_user( kbuf,user_buffer, len ))  
  	...
  	kbuf[len]='\0';	/*terminador*/

	/*Power off all leds from blinkstick*/
    for(i=0;i < NR_LEDS; i++){
    	messages[i][0]='\x05';
    	messages[i][1]=0x00;
    	messages[i][2]=i; 
    ...
    do{
    	led = strsep(&kbufp,separate);
    	if(len <= 1 || led == NULL)
    		break;
    	if( sscanf(led,"%d:%x",&i,&color) == 2) { 
    		if( i >= 0 && i < NR_LEDS){ 
    			messages[i][3]=((color>>16) & 0xff);
    			messages[i][4]=((color>>8) & 0xff);
    			messages[i][5]=(color & 0xff);			
    		}else{
    			printk(KERN_INFO "Wrong led number!.\n");
    			return -EINVAL;
    		}
    	}
    	else{
